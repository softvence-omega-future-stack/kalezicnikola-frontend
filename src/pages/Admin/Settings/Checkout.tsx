...
const Checkout: React.FC = () => {
  ...
  return (
    <div className=" bg-[#FAFAFA] rounded-2xl p-4 sm:p-6 lg:p-8">
      <div className="">
        
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-2xl font-semibold text-[#171C35] mb-1">Request</h1>
  ...
  <table className="min-w-full  border-spacing-x-4">
    <thead>
      <tr className="bg-[#FAFAFA]">
        <th className=" px-4 py-4 text-left text-base font-semibold text-[#171c35]">Name</th>
        <th className=" px-4 py-4 text-left text-base font-semibold text-[#171c35]">Doctor ID</th>
        <th className=" px-4 py-4 text-left text-base font-semibold text-[#171c35]">Plan</th>
        <th className=" px-4 py-4 text-left text-base font-semibold text-[#171c35]">Request</th>
        <th className=" px-4 py-4 text-center text-base font-semibold text-[#171c35]">Actions</th>
      </tr>
    </thead>
    ...
    <tbody className='divide-y divide-gray-200'>
      {requests.map((request) => (
        <tr key={request.id} className="hover:bg-gray-50 transition-colors ">
          ...
          {/* Plan */}
          <td className="px-4 py-4 text-sm font-medium text-[#111A2D]">{request.plan}</td>
            ...
            <div key={request.id} className="bg-white rounded-lg shadow-sm p-4 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-3 text-sm">
                <div>
                  ...
                  <p className="text-[#171c35] font-medium">{request.plan}</p>
                </div>
                <div>
              ...
              <div className="flex gap-2">
                {request.requestStatus === 'Accept' ? (
                  <button className="flex-1 px-4 py-2 bg-green-700 text-white rounded-lg text-sm font-medium">
                    Accepted
                  </button>
                ) : (
                  <>
                    ...
                    <button
                      onClick={() => handleReject(request.id)}
                      className="flex-1 px-4 py-2 border border-gray-200 text-[#171c35] rounded-lg text-sm font-medium hover:bg-gray-50 transition-colors"
                    >
                      Reject
                    </button>
                  </>
