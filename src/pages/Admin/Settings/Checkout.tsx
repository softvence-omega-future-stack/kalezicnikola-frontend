...
const Checkout: React.FC = () => {
  ...
  const getStatusText = (status: string) => {
    switch (status) {
      case 'Accept':
        return t('adminDashboard.routes.settings.tabs.checkout.status.accept');
      case 'Pending':
        return t('adminDashboard.routes.settings.tabs.checkout.status.pending');
      case 'Reject':
        return t('adminDashboard.routes.settings.tabs.checkout.status.reject');
      default:
        return status;
    }
  };
  ...
  return (
    <div className="bg-[#FAFAFA] rounded-xl md:rounded-3xl p-4 md:p-6 md:mx-0">
      {/* Header */}
      <div className="mb-6">
        <h1 className="text-xl md:text-2xl font-semibold text-headingBlack mb-1">
          {t('adminDashboard.routes.settings.tabs.checkout.title')}
        </h1>
        <p className="text-sm text-[#667085]">{t('adminDashboard.routes.settings.tabs.checkout.description')}</p>
      </div>
          ...
          <table className="min-w-full border-spacing-x-4">
            <thead>
              <tr className="bg-[#FAFAFA]">
                <th className="px-4 py-4 text-left text-base font-semibold text-headingBlack">
                  {t('adminDashboard.routes.settings.tabs.checkout.tableHeaders.name')}
                </th>
                <th className="px-4 py-4 text-left text-base font-semibold text-headingBlack">
                  {t('adminDashboard.routes.settings.tabs.checkout.tableHeaders.doctorId')}
                </th>
                <th className="px-4 py-4 text-left text-base font-semibold text-headingBlack">
                  {t('adminDashboard.routes.settings.tabs.checkout.tableHeaders.plan')}
                </th>
                <th className="px-4 py-4 text-left text-base font-semibold text-headingBlack">
                  {t('adminDashboard.routes.settings.tabs.checkout.tableHeaders.request')}
                </th>
                <th className="px-4 py-4 text-center text-base font-semibold text-headingBlack">
                  {t('adminDashboard.routes.settings.tabs.checkout.tableHeaders.actions')}
                </th>
              </tr>
            </thead>
                    ...
                    <div className="flex items-center justify-center gap-3">
                      {request.requestStatus === 'Accept' ? (
                        <span className="px-5 py-1.5 bg-[#008933] text-white rounded-full text-sm font-medium min-w-[110px]">
                          {t('adminDashboard.routes.settings.tabs.checkout.status.acceptedButton')}
                        </span>
                      ) : (
                        <>
                          <button
                          ...
                          >
                            {t('adminDashboard.routes.settings.tabs.checkout.status.approveButton')}
                          </button>
                          <button
                            onClick={() => handleReject(request.id)}
                            className="px-5 py-1.5 border border-gray-200 text-[#171c35] rounded-full text-sm font-medium transition-colors"
                          >
                            {t('adminDashboard.routes.settings.tabs.checkout.status.rejectButton')}
                          </button>
                        </>
                      )}
                    </div>
        ...
        {requests.map((request) => (
          <div key={request.id} className="bg-white rounded-lg shadow-sm p-4 space-y-4">
            ...
            <div className="grid grid-cols-2 gap-3 text-sm">
              <div>
                <p className="text-gray-600 mb-1">{t('checkout.planLabel')}</p>
                <p className="text-headingBlack font-medium">{request.plan}</p>
              </div>
              <div>
                <p className="text-gray-600 mb-1">{t('checkout.requestLabel')}</p>
