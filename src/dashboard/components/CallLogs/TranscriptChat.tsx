...
const TranscriptChat: React.FC = () => {
  ...
  const messages: Message[] = [
    ...
    { id: 3, sender: 'Floyd Miles', userId: '000032', text: 'Hello this is Floyd from USA. How are you doing?', avatar: 'https://i.ibb.co/XZV3J9Gd/tabler-list.png' },
    { id: 4, sender: 'Floyd Miles', userId: '000032', text: 'Hello this is Floyd from USA. How are you doing?' },
    { id: 5, sender: 'user', userId: '', text: 'I am good. How can I help you?' }
  ];

  const copyMessage = (id: number, text: string) => {
    navigator.clipboard.writeText(text);
    setCopiedId(id);
    setTimeout(() => setCopiedId(null), 1500); // 1.5 second পর auto hide
  };
  ...
  />
        ...
        <div className="flex flex-col gap-0">
          {messages.map((msg) => {
            ...
            return (
              <div key={msg.id} className={`flex ${isFloyd ? 'justify-start' : 'justify-end'} gap-3`}>
                {/* Avatar */}
                {isFloyd && (
                  <div className="w-12 h-12 border border-gray-100 rounded-full flex items-center justify-center flex-shrink-0">
                    {msg.avatar ? (
                      ...
                      <span className="flex items-center justify-center h-12 w-12 bg-[#E8E8E8] rounded-full">
                        <svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 12 13" fill="none" className="h-6 w-6">
                ...
                <div className="flex-1 relative group">
                  <div className={`border border-gray-50 p-2 md:p-4 rounded-2xl bg-white flex items-center ${isFloyd ? 'justify-between' : 'justify-center md:justify-end'}`}>
                    ...
                    {copiedId === msg.id && (
                      <span className="absolute -top-5 right-0 bg-black text-white text-xs px-2 py-0.5 rounded-full">Copied</span>
                    )}
