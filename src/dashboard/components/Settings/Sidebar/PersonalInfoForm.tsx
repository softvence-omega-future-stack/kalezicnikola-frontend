...
import { updateUser } from "@/store/features/auth/auth.slice";
...
useEffect(() => {
  const fetchUserData = async () => {
    try {
      ...
      const photoUrl = profile.photo ? `${baseUrl}${profile.photo}` : karen;
...
const handleImageChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
  ...
  try {
    const response = await axios.patch( 
      `${import.meta.env.VITE_API_URL}/doctor/update-my-profile`,
      formData,
      {
        headers: {
    ...
    // Postman response অনুযায়ী ডাটা path: data.profile.photo
    const relativePath = response.data.data.profile.photo; 
    const fullUrl = `${import.meta.env.VITE_API_URL.replace('/api/v1', '')}${relativePath}`;

    setFormData((prev) => ({ ...prev, profilePic: fullUrl }));
    dispatch(updateUser({ photo: relativePath }));
...
//     setSelectedImage(file); // Save for upload
...
//         imgForm.append("photo", selectedImage);

//         await axios.patch(
//           `${import.meta.env.VITE_API_URL}/doctor/update-profile`,
//           imgForm,
//           {
//             headers: {
//               Authorization: `Bearer ${accessToken}`,
