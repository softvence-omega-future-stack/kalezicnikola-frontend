...
export default function TaskList() {
  ...
  // Filtered columns based on selected filters
    ...
    return columns.map(col => {
      ...
      if (selectedPriority !== 'all') {
        ...
        filteredTasks = filteredTasks.filter(
          task => task.priority === priorityMap[selectedPriority]
        );
      }
      ...
      if (selectedStatus !== 'all') {
        return col.id === selectedStatus;
      }
      return true;
    });
  }, [columns, selectedStatus, selectedPriority]);
    ...
    const fetchTasks = async () => {
      try {
        ...
        const response = await axios.get(
          `${import.meta.env.VITE_API_URL}/doctor/task/all?page=1&limit=100`,
          {
            headers: {
  ...
  const isFiltered = selectedStatus !== 'all' || selectedPriority !== 'all';
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
          <div>
            ...
            <p className="text-base font-medium text-[#111A2D] mt-1">
              {filteredColumns.reduce((acc, col) => acc + col.count, 0)} {t("dashboard.routes.taskList.header.totalTask")}
              {isFiltered && (
            ...
            <div className="p-3 space-y-3 overflow-y-auto">
              {column.tasks.length === 0 ? (
                <div className="text-center py-8 text-gray-400 text-sm">
                  {isFiltered ? 'No tasks match the filter' : 'No tasks'}
                </div>
              ) : (
