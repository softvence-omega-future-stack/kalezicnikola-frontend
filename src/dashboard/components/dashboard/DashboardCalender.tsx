...
const DashboardCalendar: React.FC<DashboardCalendarProps> = ({
  ...
  // -----------------------------
  // Normalized & filtered appointments
  // -----------------------------
  const normalizedAppointments = useMemo(
    () => appointments.map(a => ({ ...a, date: new Date(a.appointmentDate) })),
    [appointments]
  );

  const filterByDay = useMemo(
    () => normalizedAppointments.filter(a => a.date.toDateString() === selectedDate.toDateString()),
      ...
      {isModalOpen && (
        <NewAppointmentModal
          onClose={() => {
            ...
            fetchAppointments(); // Refresh after modal close
          }}
        />
      )}
    </div>
